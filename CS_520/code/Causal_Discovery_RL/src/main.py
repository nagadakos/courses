...
def main():
    ...
    res_dir = '{}/results'.format(output_dir)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for i in (range(1, config.nb_epoch + 1)):
            ...
            if (i+1) % lambda_iter_num == 0:
                ...
                np.save('{}/{}_prediction.npy'.format(res_dir,i), graph_batch.T)
                np.save('{}/{}_prediction_pruned.npy'.format(res_dir,i), graph_batch_pruned.T)
                #np.save('{}/{}_truth.npy'.format(res_dir,i), training_set.true_graph)
